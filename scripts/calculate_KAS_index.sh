...
exampleHelp="Example: nohup KAS-Analyzer KASindex -o KAS-seq_expression -t 10 -s mm10 -r promoter -l labels.txt -k KAS-seq_data.txt &"
...
if test -z $prefix ;then
   echo ""	
   echo "Please input the prefix (basename) of 'KAS-Analyzer KASexpre' output files."	
   echo ""
   exit 1
fi   
...
if [[ $regions == "promoter" ]] || [[ $regions == "genebody" ]] ;then
   ...
   do
   j=$((i-1))	   
   paste ${prefix}_on_${assemblyid}_Refseq.gene.filter.${j}.without_header.txt ${prefix}_on_${assemblyid}_Refseq.gene.filter.KAS-seq.${i} > ${prefix}_on_${assemblyid}_Refseq.gene.filter.${i}.without_header.txt
   done
