...
exampleHelp="Example: nohup KAS-Analyzer SST_enhancer -o KAS-seq_SST_enhancers -t 10 -s mm10 -e H3K27ac_enhancers.bed -p KAS-seq_peaks.bed -k KAS-seq.rep1.bigWig,KAS-seq.rep2.bigWig &"
...
KASseqHelp="-k [KAS-seq]: please specify the bigWig file of KAS-seq data used for single-stranded transcribing enhancers (SST_enhancers) identification. e.g. KAS-seq.rep1.bigWig,KAS-seq.rep2.bigWig. REQUIRED."
...
if test -z $enhancer ;then
    echo ""
    echo "Please specify the enhancer file used for single-stranded transcribing enhancers identification. REQUIRED. -e [enhancer]"
    echo ""
    exit -1
fi
...
if test -z $KASseq ;then
    echo ""
    echo "Please specify the bigWig file of KAS-seq data used for single stranded transcribing enhancers identification. REQUIRED. -k [KAS-seq]"
    echo ""
    exit -1
fi
