...
KO.rep1
KO.rep2                     ---labels.txt"
KASseqHelp="-k [KAS-seq]: please input the text file containing the index bam files of (sp)KAS-seq data that used for differential KAS-seq analysis. REQUIRED.
Example:
KAS-seq_WT.rep1.bam
KAS-seq_WT.rep2.bam
KAS-seq_KO.rep1.bam
...
if test -z $KASseq ;then
   echo ""
   echo "Please input the text file containing indexed bam files of (sp)KAS-seq data that used for differential KAS-seq analysis. -k [KAS-seq] "
   echo ""
   exit -1
fi
...
# normalize the bam files using RPKM and output bigWig files.
cat /dev/null > ${prefix}.KAS-seq.RPKM.bigWig.txt
for ((i=1; i<=${number_of_samples}; i++))
do
sample_selected=$(sed -n ''$i'p' $KASseq)
echo "Generate the new index of $sample_selected ..."
echo ""
...
echo ""
bamCoverage -b $sample_selected --outFileFormat bigwig -p $threads -bl ${SH_SCRIPT_DIR}/../blacklist/${assemblyid}-blacklist.bed --ignoreDuplicates --effectiveGenomeSize $genomesize --normalizeUsing RPKM -o ${sample_selected}.bigWig > /dev/null 2>&1
...
if [[ $regions == "bin" ]]; then
   ...
   echo ""
   paste ${prefix}_on_1kb_bins.average ${prefix}_on_1kb_bins.bed | awk '$1>=2 {print $0}' > ${prefix}_on_1kb_bins.filter.bed
   ...
   if [[ $number_of_samples == 2 ]] ;then
      ...
      sed "s/\"//g" KAS-seq_DESeq2_output.csv | sed "s/\,/\t/g" > ${prefix}_KAS-seq_bins_DESeq2_output.txt
      sed "s/\"//g" DE.KAS-seq_DESeq2_Fold_padj0.05_output.csv | sed "s/\,/\t/g" > ${prefix}_DE.KAS-seq_bins_DESeq2_fold${foldchange}_padj0.05_output.txt
   ...
   if [[ $number_of_samples == 2 ]] ;then
      ...
      cat ${prefix}.header.txt ${prefix}_gain_of_KAS-seq.without_header.txt > ${prefix}_gain_of_KAS-seq_peaks.txt
      cat ${prefix}.header.txt ${prefix}_loss_of_KAS-seq.without_header.txt > ${prefix}_loss_of_KAS-seq_peaks.txt
   ...
   echo ""
   multiBigwigSummary BED-file --bwfiles $KASseq_list --BED ${SH_SCRIPT_DIR}/../Annotation/${assemblyid}_Refseq.${regions}.bed --labels $labels_list -p $threads -out ${prefix}_on_${assemblyid}_Refseq.${regions}.npz --outRawCounts ${prefix}_on_${assemblyid}_Refseq.${regions}.tab > /dev/null 2>&1
...
# filter the ${regions} with normalized KAS-seq expression over 2.
...
# clean up the intermediate files.
   ...
   # filter the promoter or genebody with averaged KAS-seq expression over 2.
