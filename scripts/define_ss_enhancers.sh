...
exampleHelp="Example: nohup KAS-pipe2 ss_enhancer -o KAS-seq_ss_enhancers -t 10 -s mm10 -e H3K27ac_enhancers.bed -p KAS-seq_peaks.bed -k KAS-seq.rep1.bam,KAS-seq.rep2.bam &"
...
KASseqHelp="-k [KAS-seq]: please specify the indexed bam file of KAS-seq data used for single stranded (ss) enhancers identification. e.g. KAS-seq.rep1.bam,KAS-seq.rep2.bam. REQUIRED."
...
if test -z $KASseq ;then
    echo ""
    echo "Please specify the indexed bam file of KAS-seq data used for single stranded (ss) enhancers identification. REQUIRED. -k [KAS-seq]"
    echo ""
    exit -1
fi
