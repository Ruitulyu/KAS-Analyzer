...
KO.rep1
KO.rep2                       ---labels.txt"
KASseqHelp="-k [KAS-seq]: please input the text file containing the bigWig files of (sp)KAS-seq data that used to calculate elongation index. REQUIRED.
Example:
...
if [[ $regions == "promoter" ]] || [[ $regions == "genebody" ]] ;then
   ...
   echo "Calculating the elongation index on ${assemblyid}_Refseq.${regions}.bed ..."
   ...
   # filter the ${regions} with elongation index over 5.
   ...
   # calculate the average elongation index of every single row in the table.
   echo "Calculate the elongation index on ${assemblyid} Refseq promoter and genebody ..."
   echo ""
   ...
   # filter the promoter or genebody with elongation index lower than 5.
   ...
   cat ${prefix}.header.txt ${prefix}_on_${assemblyid}_Refseq.gene.filter.without_header.txt > ${prefix}_on_${assemblyid}_Refseq_gene_elongation_index.txt
   echo "done."
   echo ""
