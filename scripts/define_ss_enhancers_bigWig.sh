#!/bin/bash
# 'KAS-pipe2 ss_enhancer' was developed by Ruitu Lyu on 1-22-2022.
...
exampleHelp="Example: nohup KAS-pipe2 ss_enhancer -o KAS-seq_ss_enhancers -t 10 -s mm10 -e H3K27ac_enhancers.bed -p KAS-seq_peaks.bed -k KAS-seq.rep1.bigWig,KAS-seq.rep2.bigWig &"
...
KASseqHelp="-k [KAS-seq]: please specify the bigWig file of KAS-seq data used for single stranded (ss) enhancers identification. e.g. KAS-seq.rep1.bigWig,KAS-seq.rep2.bigWig. REQUIRED."
...
# if no parameters was provided, 'KAS-pipe2 ss_enhancer' will print the help.
...
if test -z $enhancer ;then
    echo ""
    echo "Please specify the enhancer file used for single stranded (ss) enhancers identification. REQUIRED. -e [enhancer]"
    echo ""
    exit -1
fi
...
if test -z $KASseq ;then
    echo ""
    echo "Please specify the bigWig file of KAS-seq data used for single stranded (ss) enhancers identification. REQUIRED. -k [KAS-seq]"
    echo ""
    exit -1
fi
...
number_of_samples=$( awk 'END {print NF}' .KASseq.txt )
...
labels_basename=$( basename ${sample_selected} .bigWig)
echo ${labels_basename} >> .labels.txt
done

labels_list=$(awk '{for(i=1;i<=NF;i++) a[i,NR]=$i}END{for(i=1;i<=NF;i++) {for(j=1;j<=NR;j++) printf a[i,j] " ";print ""}}' .labels.txt)
rm -f .labels.txt
rm -f .KASseq.txt
echo "done."
echo ""
...
multiBigwigSummary BED-file --bwfiles $KASseq_list --BED ${enhancer}.KAS.distal.bed --labels $labels_list -p $threads -out ${prefix}_on_${enhancer}.KAS.distal.npz --outRawCounts ${prefix}_on_${enhancer}.KAS.distal.tab

multiBigwigSummary BED-file --bwfiles $KASseq_list --BED ${enhancer}.KAS.distal.left.bed --labels $labels_list -p $threads -out ${prefix}_on_${enhancer}.KAS.distal.left.npz --outRawCounts ${prefix}_on_${enhancer}.KAS.distal.left.tab

multiBigwigSummary BED-file --bwfiles $KASseq_list --BED ${enhancer}.KAS.distal.right.bed --labels $labels_list -p $threads -out ${prefix}_on_${enhancer}.KAS.distal.right.npz --outRawCounts ${prefix}_on_${enhancer}.KAS.distal.right.tab
...
echo "'KAS-pipe2 ss_enhancer' run successfully."
