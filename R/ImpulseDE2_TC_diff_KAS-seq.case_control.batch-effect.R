...
coldata <- as.data.frame(read.table(args[2], header = T))
...
write.csv(objectImpulseDE2$dfImpulseDE2Results, "KAS-seq_case_control_TC_ImpulseDE2_nor-batch_output.csv")

## -----------------------------------------------------------------------------
# Visualization of global TC KAS-seq patterns via a heatmap

# Visualize global TC KAS-seq patterns and save it as png plot.
png(file="KAS-seq_case_control_TC_ImpulseDE2_nor-batch_heatmap.png", bg="transparent")
plotHeatmap(
...
png(file="KAS-seq_case_control_TC_ImpulseDE2_nor-batch_heatmap.svg", bg="transparent")
plotHeatmap(
  objectImpulseDE2       = objectImpulseDE2,
  strCondition           = c("case","control"),
  boolIdentifyTransients = TRUE,
